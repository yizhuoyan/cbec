...
import static com.neusoft.cq.cbec.common.dao.support.JDBCUtil.generateInsertSQL;
import static com.neusoft.cq.cbec.common.dao.support.JDBCUtil.generateLikeSql;
...
public abstract class SingleTableDaoSupport<T> implements CRUDDao<T> {
    ...
    @Override
    public Object[] selectColumns2Array(String uniqueColumnName, Object uniqueColumnValue,final String... selectColumns)
    		throws Exception {
    	 Sql sql = Sql.select(join(selectColumns, ","))
                 .from(tableName)
                 .where(uniqueColumnName, "=?")
                 .limit(0, 1);
         return  executeQuerySql(sql,rs->{
        	 Object[] result=new Object[selectColumns.length];
        	 for(int i=selectColumns.length;i-->0;) {
    ...
    @Override
    public Map<String, Object> selectColumns2Map(String uniqueColumnName, Object uniqueColumnValue,
    	 ...
    	 Sql sql = Sql.select(join(selectColumns, ","))
                 ...
                 .limit(0, 1);
         return  executeQuerySql(sql,rs->{
        	 Map<String,Object> result=new HashMap<>(selectColumns.length);
        	 for(int i=selectColumns.length;i-->0;) {
        		 result.put(selectColumns[i],rs.getObject(i+1));
        	 }
        	 return result;
         }, uniqueColumnValue);
    }
    ...
    public <R> R selectOneColumn(String uniqueColumnName, Object uniqueColumnValue, String selectColumnName)
    		throws Exception {
    	 Sql sql = Sql.select(selectColumnName)
                 .from(tableName)
                 .where(uniqueColumnName, "=?")
                 .limit(0, 1);
         return (R)executeQuerySql(sql,rs->{
    ...
    protected final void executeInsert(String tableDesc, Object... values) throws Exception {
        JDBCUtil.executeInsert(getConnection(), tableDesc, values);
    }

    protected final int executeDelete(String tableDesc, Object... values) throws Exception {
        return JDBCUtil.executeDelete(getConnection(), tableDesc, values);
    }


    protected final <T> T executeQuerySql(Object sql, RowMapper<T> mapper, Object... params) throws Exception {

        return JDBCUtil.executeQuerySql(getConnection(), sql, mapper, params);
    }


    protected final int executeCountSql(Object sql, Object... params) throws Exception {
        return JDBCUtil.executeCountSql(getConnection(), sql, params);
    }

    protected final <T>List<T> executesQuerySql(Object sql, RowMapper<T> mapper, Object... params) throws Exception {
        return JDBCUtil.executesQuerySql(getConnection(), sql, mapper, params);
    }

    protected final void executesQuerySql(Object sql, RowMapper<T> mapper, List<T> result, Object... params) throws Exception {
        JDBCUtil.executesQuerySql(getConnection(), sql, mapper, result, params);
    }

    protected final int executeUpdateSql(Object sql, Object... params) throws Exception {
        return JDBCUtil.executeUpdateSql(getConnection(), sql, params);
    }
